                    ...
                    <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                            <div class="iq-header-title">
                                ...
                                <p class="text-dark">Bulan {{ $currentMonthtext }}, Semester
                        ...
                        <div class="d-flex justify-content-between align-items-center py-3 px-2">
                            <div class="row col-sm-12">
                                <div class="col-sm-3">
                                    <label for="filter_status">Status Pembayaran</span></label>
                                ...
                                <div class="col-sm-3">
                                    <label for="filter_semester">Semester</span></label>
                                ...
                                <div class="col-sm-2">
                                    <label for="filter_bulan">Bulan</span></label>
                                    <select class="form-control" name="filter_bulan" id="filter_bulan">
                                    </select>
                                </div>
                                <div class="col-sm-2">
                                    <label for="nama_santri">Tahun</span></label>
        ...
        <div class="modal fade" id="deleteModal{{ $iuran->id_pembayaran }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ url('/admin/iuran_bulanan/delete/' . $iuran->id_pembayaran . '/action') }}" id="deleteForm"
                        method="post">
                        @csrf
                        @method('DELETE')
    ...
    <script>
        $(document).ready(function () {
            $('#tableIuranBulanan').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ secure_url('admin/iuran_bulanan') }}",
                    data: function (d) {
                            ...
                            if (full.status_pembayaran == 'belum_lunas') {
                                if (full.jumlah_bayar == 0) {
                                    ...
                                    return '<div class="d-flex flex-column">' +
                                        ...
                                        '<a class="badge badge-pill badge-success p-2 mt-2" title="Info cicilan" href="/admin/iuran_bulanan/cicilan/' +
                                        full
                                            .id_pembayaran + '/bayar">' +
                                        'Detail cicilan' +
                                        '</a>' +
            ...
            function preparationBulanOptions() {
                const selectedSemester = $('#filter_semester').val();
                const options = bulanMap[selectedSemester];

                for (const [value, label] of Object.entries(options)) {
                    const isSelected = parseInt(value) === parseInt(currentMonth) ? 'selected' : '';
                    $('#filter_bulan').append(`<option value="${value}" ${isSelected}>${label}</option>`);
                }
            }

            preparationBulanOptions();

            function updateBulanOptions() {
                const selectedSemester = $('#filter_semester').val();
                const options = bulanMap[selectedSemester];
                $('#filter_bulan').html('<option value="">-- Pilih Bulan --</option>');

                for (const [value, label] of Object.entries(options)) {
                    $('#filter_bulan').append(`<option value="${value}">${label}</option>`);
                }
            }
            ...
            $('#filter_status, #filter_bulan, #filter_tahun').on('change', function () {
                $('#tableIuranBulanan').DataTable().ajax.reload(null, false);
            });
        });
    </script>
