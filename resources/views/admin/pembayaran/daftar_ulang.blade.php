...
@section('content')
    ...
    <!-- Modal Delete -->
    @foreach ($pembayarans_lunas as $daftar_ulang)
        <div class="modal fade" id="deleteModal{{ $daftar_ulang->id_pembayaran }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ url('/admin/daftar_ulang/delete/' . $daftar_ulang->id_pembayaran . '/action') }}"
                                    ...
                                    <table class="table table-borderless mb-0">
                                        <tbody class="text-secondary">
                                            ...
                                            <tr>
                                                ...
                                                <td>:
                                                    {{ \Carbon\Carbon::parse($daftar_ulang->tanggal_pembayaran)->format('d-m-Y') }}
                                                </td>
                                            </tr>
                                            <tr>
                                                <td><strong>Jam Pembayaran</strong></td>
                                                <td>:
                                                    {{ \Carbon\Carbon::parse($daftar_ulang->tanggal_pembayaran)->format('H:i:s') }}
                                                </td>
                                            </tr>
                                            <tr>
                                                <td><strong>Jumlah Pembayaran</strong></td>
                                                <td>: Rp.
                                                    {{ number_format($daftar_ulang->jumlah_pembayaran, 0, ',', ',') }}
                                                </td>
                                            </tr>

                                            <tr>
                                                <td><strong>Diterima Oleh</strong></td>
                                                <td>: {{ $daftar_ulang->user->nama_admin }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
    ...
    <script>
        $(document).ready(function () {
            $('#tableDaftarUlang').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ secure_url('admin/daftar_ulang') }}",
                    data: function (d) {
                            ...
                            if (full.status_pembayaran == 'belum_lunas') {
                                if (full.jumlah_bayar == 0) {
                                    ...
                                    return '<div class="d-flex flex-column">' +
                                        ...
                                        '<a class="badge badge-pill badge-success p-2 mt-2" title="Info cicilan" href="/admin/daftar_ulang/cicilan/' +
                                        full
                                            .id_pembayaran + '/bayar">' +
                                        'Detail cicilan' +
                                        '</a>' +
            ...
            $('#filter_status, #filter_semester, #filter_tahun').on('change', function () {
                $('#tableDaftarUlang').DataTable().ajax.reload(null, false);
            });
        });
    </script>
