...
class AdminDaftarUlangController extends Controller
{
    public function index(Request $request)
    {
        ...
        $createdTerbaru = Pembayaran::where('jenis_pembayaran', 'daftar_ulang')
    ...
    {
        $data['title'] = 'Cicilan Daftar Ulang';
        ...
        return view('admin.pembayaran.cicilan.cicilan_daftarUlang', [
    ...
    public function add_cicilan(Request $request)
    {
        ...
        if ($total_bayar > $pembayaran->jumlah_pembayaran) {
            ...
            return redirect()->route('daftar_ulang')->with('success', 'Pembayaran daftar ulang lunas, kembalian Rp' . number_format($kembalian, 0, ',', '.'));
        }
        ...
        switch (true) {
            case $total_bayar == $pembayaran->jumlah_pembayaran:
                ...
                return redirect()->route('daftar_ulang')->with('success', 'Pembayaran daftar ulang Lunas');
            ...
            default:
                ...
                return redirect()->route('cicilan_daftar_ulang_detail', ['id' => $request->id_pembayaran])->with('success', 'Data cicilan berhasil ditambahkan');
        }
    }
    ...
    public function delete_cicilan(Request $request, $id_cicilan_pembayarans)
    {
        ...
        if ($pembayaran->jumlah_bayar == 0) {
            ...
            // Simpan perubahan ke database
            $pembayaran->save();
            return redirect()->route('daftar_ulang', ['id' => $request->id_pembayaran])->with('success', 'Pembayaran cicilan daftar ulang dibatalkan');
        }

        return redirect()->route('cicilan_daftar_ulang_detail', ['id' => $request->id_pembayaran])->with('success', 'Pembayaran cicilan daftar ulang dibatalkan');
    }
}