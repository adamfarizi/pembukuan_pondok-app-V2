...
class AdminIuranBulananController extends Controller
{
    public function index(Request $request)
    {
        ...
        $currentMonthtext = Carbon::now()->locale('id')->translatedFormat('F');
        ...
        if ($request->ajax()) {
            ...
            // Filter Bulan
            if ($request->filled('filter_bulan') && $request->filter_bulan !== $currentMonth) {
                $data->whereMonth('created_at', $request->filter_bulan);
            } else {
                $data->whereMonth('created_at', $currentMonth);
            }
    ...
    public function edit(Request $request, $id_santri)
    {
        ...
        if ($statusPotonganHarga == "true" && $pembayaran) {
            ...
            switch ($request->jenis_bayar) {
                case 'lunas':
                    // Jika tidak ada potongan harga atau potongan tidak diterapkan, proses normal (lunas tanpa potongan)
                    $pembayaran->jumlah_bayar = $pembayaran->jumlah_pembayaran;
                    $pembayaran->status_pembayaran = 'lunas';
                    break;
    ...
    {
        $data['title'] = 'Cicilan Iuran Bulanan';
        ...
        return view('admin.pembayaran.cicilan.cicilan_iuran', [
            'bulan' => Carbon::parse($pembayarans->created_at)->locale('id')->translatedFormat('F'),
            'currentSemester' => Carbon::parse($pembayarans->created_at)->year,
            'pembayarans' => $pembayarans,
    ...
    public function add_cicilan(Request $request)
    {
        ...
        if ($total_bayar > $pembayaran->jumlah_pembayaran) {
            ...
            return redirect()->route('iuran_bulanan')->with('success', 'Pembayaran iuran bulanan lunas, kembalian Rp' . number_format($kembalian, 0, ',', '.'));
        }
        ...
        switch (true) {
            case $total_bayar == $pembayaran->jumlah_pembayaran:
                ...
                return redirect()->route('iuran_bulanan')->with('success', 'Pembayaran iuran bulanan Lunas');
            ...
            default:
                ...
                return redirect()->route('cicilan_iuran_bulanan_detail', ['id' => $request->id_pembayaran])->with('success', 'Data cicilan berhasil ditambahkan');
        }
    }
    ...
    public function delete_cicilan(Request $request, $id_cicilan_pembayarans)
    {
        ...
        if ($pembayaran->jumlah_bayar == 0) {
            ...
            // Simpan perubahan ke database
            $pembayaran->save();
            return redirect()->route('iuran_bulanan', ['id' => $request->id_pembayaran])->with('success', 'Pembayaran cicilan iuran bulanan dibatalkan');
        }

        return redirect()->route('cicilan_iuran_bulanan_detail', ['id' => $request->id_pembayaran])->with('success', 'Pembayaran cicilan iuran bulanan dibatalkan');
    }
}
